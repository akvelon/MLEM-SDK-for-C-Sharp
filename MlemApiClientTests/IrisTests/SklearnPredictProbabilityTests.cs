...
{
    public class SklearnPredictProbabilityTests : BaseTests
    {
        [Test]
        public async Task PositiveTest()
        {
            var result = await _client.SklearnPredictProbabilityAsync(
                new List<Iris>
                {
                    new Iris
                    {
        ...
        {
            Assert.ThrowsAsync<ArgumentNullException>(() => _client.SklearnPredictProbabilityAsync(null));
        }

        [Test]
        public void EmptyValueTest()
        {
            Assert.ThrowsAsync<ArgumentException>(() => _client.SklearnPredictProbabilityAsync(new List<Iris>()));
        }
    }
}